...
static void* task_wrapper(void* arg) {
    threadx_task_t* task = (threadx_task_t*)arg;
    ...
    uint32_t stack_size, rtos_task_fn_t entry, void* param)
{
    ...
    threadx_task_t* task = calloc(1, sizeof(threadx_task_t));
...
rtos_status_t rtos_task_delete(rtos_handle_t handle) {
    ...
    threadx_task_t* task = (threadx_task_t*)handle;
...
void rtos_task_check_suspend(threadx_task_t* task) {
    if (!task) return;
